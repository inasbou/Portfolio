...
import { Mail, Github, Linkedin, ExternalLink, Award, Briefcase, Code, Send, ChevronLeft, ChevronRight, BrainCircuit, Terminal, Globe, Cpu, Server, Database, Sparkles, Zap } from 'lucide-react';
const Portfolio = () => {
  ...
  const projects = [
    { 
      ...
      tech: ["LLM", "RAG", "Kubernetes","WebDev"], 
    ...
    { 
      ...
      desc: " Full-stack University administration platform ", 
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-slate-900 font-sans selection:bg-violet-100">
      // --- MODERN NAVIGATION --- 
      <nav className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-2xl border-b border-slate-200/50 px-8 py-5 flex  items-center">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-40 space-y-40 pb-20">
        ...
        <section id="about" className="max-w-4xl relative">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet-50 text-violet-600 text-[10px] font-black uppercase tracking-widest mb-8 border border-violet-100 shadow-sm">
            <Sparkles size={12} /> State Engineer in AI & Autonomous Systems
          </div>
          ...
          <p className="text-2xl text-slate-500 leading-relaxed max-w-2xl font-medium italic">
            "Bridging the gap between <span className="text-slate-900 font-bold not-italic">Large Language Models</span> and scalable <span className="text-slate-900 font-bold not-italic">Cloud-Native architectures</span>."
          </p>
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
                { title: "AI & Research", icon: <BrainCircuit className="text-violet-500"/>, bg: "bg-violet-50", border: "border-violet-100", skills: ["LLMs", "RAG", "Fine-tuning", "PyTorch", "NLP"] },
                { title: "Web Architecture", icon: <Terminal className="text-blue-500"/>, bg: "bg-blue-50", border: "border-blue-100", skills: ["React", "TypeScript", "Node.js", "FastAPI", "MERN"] },
                { title: "Cloud & Ops", icon: <Server className="text-emerald-500"/>, bg: "bg-emerald-50", border: "border-emerald-100", skills: ["Docker", "Kubernetes", "CI/CD", "Nginx", "Git"] },
                { title: "Data Systems", icon: <Database className="text-amber-500"/>, bg: "bg-amber-50", border: "border-amber-100", skills: ["PostgreSQL", "MongoDB", "Neo4j", "InfluxDB", "Redis"] },
            ].map((cat, i) => (
      ...
      <section id="projects" className="max-w-6xl mx-auto px-6 pb-32">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {projects.map((p) => (
            <div
              key={p.name}
              className="bg-white rounded-3xl p-8 shadow-lg hover:-translate-y-2 hover:shadow-2xl transition-all"
            >
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 bg-indigo-600 text-white py-3 rounded-full font-black uppercase text-xs tracking-widest hover:bg-indigo-700 transition"
                >
                  Show Project
                </button>
                ...
                <a
                  href={p.link}
                  target="_blank"
                  className="flex items-center justify-center px-4 rounded-full border border-slate-200 hover:bg-slate-100 transition"
                >
 ...
 {/* MODAL */}
      {activeProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur px-6">
          <div className="bg-white max-w-xl w-full rounded-[2rem] p-8 relative shadow-2xl animate-fadeIn">
            <button
            ...
            <div className="flex gap-4">
              <a
                ...
                target="_blank"
                className="flex-1 text-center bg-indigo-600 text-white py-4 rounded-full font-black uppercase text-xs tracking-widest hover:bg-indigo-700 transition"
              >
                View Repository
              </a>
              ...
              <button
                onClick={() => setActiveProject(null)}
                className="flex-1 border border-slate-200 py-4 rounded-full font-black uppercase text-xs tracking-widest hover:bg-slate-100 transition"
              >
                Close
              </button>
            </div>
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-16">
              <div className="relative pl-12 border-l-4 border-violet-600/20">
                ...
                <p className="text-violet-600 text-[10px] font-black uppercase mt-2 mb-4 tracking-widest">L3i Laboratory, France • 2024 - 2025</p>
        ...
        <section id="contact" className="text-center py-32 bg-gradient-to-b from-white to-violet-50 border border-violet-100 rounded-[5rem] shadow-xl shadow-violet-100/50 relative overflow-hidden">
          ...
          <p className="text-slate-500 mb-12 font-bold uppercase text-[10px] tracking-[0.4em]">Available for Engineering and AI Research roles</p>
